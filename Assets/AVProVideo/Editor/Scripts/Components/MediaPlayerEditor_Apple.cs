...
namespace RenderHeads.Media.AVProVideo.Editor
{
	...
	public partial class MediaPlayerEditor : UnityEditor.Editor
	{
		private void OnInspectorGUI_Override_Apple(Platform platform)
		{
			...
			{
				EditorGUILayout.PropertyField(preferredMaximumResolutionProp, new GUIContent("Preferred Maximum Resolution", "The desired maximum resolution of the video (HLS only)."));
			...
			SerializedProperty preferredPeakBitRateProp = serializedObject.FindProperty(optionsVarName + "._preferredPeakBitRate");
			SerializedProperty preferredPeakBitRateUnitsProp = serializedObject.FindProperty(optionsVarName + "._preferredPeakBitRateUnits");
			...
			SerializedProperty preferredForardBufferDurationProp = serializedObject.FindProperty(optionsVarName + "._preferredForwardBufferDuration");
			if (preferredForardBufferDurationProp != null)
			{
				EditorGUILayout.PropertyField(preferredForardBufferDurationProp, new GUIContent("Preferred Forward Buffer Duration", "The duration in seconds the player should buffer ahead of the playhead to prevent stalling. Set to 0 to let the system decide."));
			}
