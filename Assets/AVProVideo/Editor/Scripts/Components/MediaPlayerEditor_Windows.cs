		...
		private void OnInspectorGUI_Override_Windows()
		{
			...
			// Media Foundation Options
			{
				...
				}
				{
					...
					SerializedProperty propUseStereoDetection = serializedObject.FindProperty(optionsVarName + ".useStereoDetection");
					if (propUseLowLatency != null)
					{
						EditorGUILayout.PropertyField(propUseStereoDetection, new GUIContent("Use Stereo Detection", "Disable if no stereo detection is required"));
					}
					{
						GUILayout.Label("Hap/NotchLC");
						EditorGUI.indentLevel++;
						SerializedProperty propUseHapNotchLC = serializedObject.FindProperty(optionsVarName + ".useHapNotchLC");
						if (propUseLowLatency != null)
						{
							EditorGUILayout.PropertyField(propUseHapNotchLC, new GUIContent("Use Hap/NotchLC", "Disable if no Hap/NotchLC playback is required"));
						}
