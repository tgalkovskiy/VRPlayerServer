...
public class Pvr_UnitySDKManager : MonoBehaviour
{
    ...
    private void CheckControllerStateForG2(string state)
    {
        ...
        {
            if (Convert.ToBoolean(Convert.ToInt16(state)) && Pvr_UnitySDKAPI.Controller.UPvr_GetControllerPower(0) == 0 && Pvr_ControllerManager.controllerlink.Controller0.Rotation.eulerAngles != Vector3.zero)
            {
        ...
        if (Pvr_UnitySDKSensor.Instance != null)
        {
            if (isHasController)
            {
                if (Pvr_UnitySDKAPI.Controller.UPvr_GetControllerState(0) == ControllerState.Connected ||
                    Pvr_UnitySDKAPI.Controller.UPvr_GetControllerState(1) == ControllerState.Connected)
                {
...
#region Unity API
    ...
    void Update()
    {
        ...
        {
            if (Pvr_UnitySDKAPI.Controller.UPvr_GetControllerPower(0) == 0 && Pvr_ControllerManager.controllerlink.controller0Connected && Pvr_ControllerManager.controllerlink.Controller0.Rotation.eulerAngles != Vector3.zero)
            {
